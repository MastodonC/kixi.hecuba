...
(deftype CsvLoader [config]
  ...
  (start [_ system]
    (let [host "localhost"
      ...
      #_(clojure.pprint/pprint programme-map)
        ...
        (let [entity-map
          ...
          (let [entity-id (get entity-map "64")]
            (doseq [device (generators/generate-device-sample entity-id 3)]
              (let [device (assoc device :readings (map jsonify (generators/generate-sensor-sample "INSTANT" 3)))]

                (println "response to creating device is"
                         @(post-resource
                           (format "http://%s:%d%s" host port (path-for (-> system :hecuba/routing :jig.bidi/routes) (-> system :amon/handlers :devices) :entity-id entity-id))
                           "application/json"
                           ;; Don't need device-id, it gets generated by liberator
                           (jsonify (dissoc device :device-id))))))))))

    system)
  (stop [_ system] system))


#_(take 10 (drop 50 (extract-data (io/file (-> user/system :jig/config :jig/components :hecuba.dev/etl :data-directory)) "properties.csv")))
