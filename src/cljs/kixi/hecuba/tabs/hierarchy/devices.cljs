...
(defn location-input [cursor owner]
  [:div
   [:div.form-group
    ...
    [:input {:value (om/get-state owner [:device :location :name])
   ...
   [:div.form-group
    ...
    [:input {:value (om/get-state owner [:device :location :latitude])
   ...
   [:div.form-group
    ...
    [:input {:value (om/get-state owner [:device :location :longitude])
...
(defn sensor-period-dropdown [owner keys]
  [:div.form-group
   [:label.control-label {:for "period-select"} "Period"]
   [:div.required
    [:select.form-control {:value (om/get-state owner keys)
      ...
      (render-state [_ state]
        (html
         (let [property_id  (:properties (om/observe owner (current-selections)))
           ...
           [:div
            [:div {:id "new-sensor-alert"} (bs/alert owner)]
            [:h3 "Add new sensor"]
            [:form.form-horizontal {:role "form"}
             [:div.col-md-6
              ...
              (bs/text-input-control owner [:sensor :type] "Type" true)
              (bs/text-input-control owner [:sensor :alias] "Header Rows")
     ...
     [:div.row
      [:div.col-md-12 {:style {:padding-bottom "10px"}}
       [:button {:type "button"
        ...
        "Delete Sensor"]]]
     (bs/text-input-control owner [:sensors sensor_id :type] "Type")
     (bs/text-input-control owner [:sensors sensor_id :alias] "Header Rows")
...
(defn synthetic-sensor-edit-div [cursor owner]
  (let [sensor_id   (:sensor_id cursor)]
    [:div
     [:li.list-group-item
      (bs/static-text cursor [:type] "Type")
      (bs/static-text cursor [:unit] "Unit")
      (bs/static-text cursor [:period] "Period")
      (bs/checkbox owner [:sensors sensor_id :actual_annual] "Calculated Field")]]))
  ...
  (reify
    ...
    (init-state [_]
      {:device (:device cursor)
       :sensors (into {} (map #(hash-map (:sensor_id %) %) (:readings (:device cursor))))})
    om/IRenderState
    (render-state [_ state]
      (let [property_id  (:properties (om/observe owner (current-selections)))
        ...
        (html
         [:div
          [:div {:id "edit-device-alert"} (bs/alert owner)]
          [:h3 "Editing device"]
          [:form {:role "form"}
           [:div.col-md-6
            [:div.form-group
            ...
            (if-not synthetic?
              [:div
               (bs/text-input-control owner [:device :description] "Unique Description")
               (bs/text-input-control owner [:device :name] "Further Description")
...
(defn new-device-form [cursor owner]
  (reify
    om/IInitState
    (init-state [_]
      {:device {} :sensor {:period "Select period"}})
    om/IRender
    (render [_]
      (html
       (let [property_id  (:properties (om/observe owner (current-selections)))
         ...
         [:div
          [:div.col-md-12
           [:div {:id "new-device_alert"} (bs/alert owner)]
           [:h3 "Add new device"]
           [:form.form-horizontal {:role "form"}
            [:div.col-md-6
             [:div.form-group
              [:div.btn-toolbar
               ...
               [:button {:type "button"
                         ...
                         :onClick (fn [_]
                                    (put! event-chan {:event :adding-device :value false}))}
                "Cancel"]]]
             (bs/text-input-control owner [:device :description] "Unique Description" true)
             (bs/text-input-control owner [:device :name] "Further Description")
             (location-input cursor owner)
             (bs/checkbox owner [:device :privacy] "Private")]]]
          [:div.col-md-12
           [:h3 "Create sensor:"]
           [:form.form-horizontal {:role "form"}
            [:div.col-md-6
