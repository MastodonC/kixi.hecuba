  ...
  `(let [options# (apply hash-map '~options)]
     (defn
       ...
       ([~'type] (submit-item (-> kixi/system :pipeline :head)
                    (merge {:type ~'type}
                           options#)))
       ([~'type ~'operation] (submit-item (-> kixi/system :pipeline :head)
                                          (merge {:type ~'type :operation ~'operation}
                                                 options#)))
       ([] (~name nil)))))
...
(defreplmethods total-usage-weekly :dest :calculated-datasets :type :total-usage :operation :weekly)
(defreplmethods total-usage-monthly :dest :calculated-datasets :type :total-usage :operation :weekly)
;; Examples:
;; (recalculate :rollups)
