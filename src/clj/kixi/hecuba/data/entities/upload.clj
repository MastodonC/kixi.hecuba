...
(defn- upload [store {:keys [dir filename entity_id public? metadata] :as item} update-fn]
  (db/with-session [session (:hecuba-session store)]
    (try
     (s3/store-file (:s3 store) item)
     (update-fn session entity_id {:path (s3/s3-key-from item) :public? public?
                                   :file_name (-> metadata :filename)}) ;; TODO call to s3-key-from is an abstraction leak.
